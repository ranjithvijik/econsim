...
[QA Status](https://github.com/ranjithvijik/econsim/actions)
...
Automated QA now includes a **Python smoke check** that installs `requirements.txt`, validates key imports (`streamlit`, `requests`, `world_bank_data`), and compiles `app.py` to catch runtime dependency regressions early.
...
- **Streamlit mirror** — `app.py` mirrors `index.html`, injects deploy-time API keys, and prefers `st.iframe` embedding (with a compatibility fallback) to stay ahead of Streamlit deprecations.
...
| Secret / env | Use                                                                                                                    |
| ------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `FRED_API`   | St. Louis Fed macro series (build-time `sed` replaces `__FRED_API_KEY_`_ inside `FRED_API_KEY_BUILD` in `index.html`). |
